...
local menu_item = SBAR.add("item", "menu_trigger", {
	position = "left",
	icon = { font = { family = "Rounded Mplus 1c Bold", size = 17.0 }, string = "禅", color = 0xffffffff },
	label = { drawing = false },
})
...
for i = 1, max_items do
	local item = SBAR.add("item", "menu." .. i, {
		...
		icon = { drawing = false },
		label = {
			font = { family = "Rounded Mplus 1c Bold", style = "Bold" },
...
for i = 1, max_items do
	...
	menu_items[i]:subscribe("mouse.exited", function()
		mouse_on_menu = false
		menu_items[i]:set({
			label = { font = { family = "Rounded Mplus 1c Bold", style = "Bold" }, color = COLORS.disabled_color },
		})
		update_state()
	end)
